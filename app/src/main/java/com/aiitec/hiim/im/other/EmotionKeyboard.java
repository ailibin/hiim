...
public class EmotionKeyboard {
    ...
    private static final String SHARE_PREFERENCE_EMOTION_HEIGHT = "emotion_height";
        ...
        mEditText.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_UP && mEmotionLayout.isShown()) {
                    ...
                    if (emotionSwitchListener != null) {
                        emotionSwitchListener.onEmotionClose();
                    }
                    //软件盘显示后，释放内容高度
    ...
    private EmotionSwitchListener emotionSwitchListener;

    public void setEmotionSwitchListener(EmotionSwitchListener emotionSwitchListener) {
        this.emotionSwitchListener = emotionSwitchListener;
    }

    public interface EmotionSwitchListener {
        /**
         * 表情面板打开
         */
        void onEmotionShow();
            ...
            public void onClick(View v) {
                if (mEmotionLayout.isShown()) {
                    ...
                    if (isSoftInputShown()) {
                        ...
                        showEmotionLayout();
                        if (emotionSwitchListener != null) {
                            emotionSwitchListener.onEmotionShow();
                        }
                        unlockContentHeightDelayed();
                    } else {
                        //两者都没显示，直接显示表情布局
...
//        mEmotionLayout.getLayoutParams().height = softInputHeight;
     ...
     * @return
     */
    public EmotionKeyboard setEmotionHeight(int height) {
        emotionHeight = height;
        if (emotionHeight > 0) {
            //保存一份到本地
            sp.edit().putInt(SHARE_PREFERENCE_EMOTION_HEIGHT, emotionHeight).apply();
        }
        return this;
    }
    ...
    private int getSupportSoftInputHeight() {
        ...
        if (softInputHeight < 0) {
            LogUtil.w("EmotionKeyboard--Warning: value of softInputHeight is below zero!");
        }
        //存一份到本地
        if (softInputHeight > 0) {
