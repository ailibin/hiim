 ...
 * 群数据结构
 */
public class GroupInfo implements Observer,TIMValueCallBack<List<TIMGroupDetailInfo>> {
    ...
    private void updateGroup(TIMGroupCacheInfo info) {
        if (groups == null || groups.get(info.getGroupInfo().getGroupType()) == null) {
            return;
        }
    ...
    public static String getTypeName(String type) {
        if (type.equals(GroupInfo.publicGroup)) {
            return App.Companion.getContext().getString(R.string.public_group);
        } else if (type.equals(GroupInfo.privateGroup)) {
            return App.Companion.getContext().getString(R.string.discuss_group);
        } else if (type.equals(GroupInfo.chatRoom)) {
            return App.Companion.getContext().getString(R.string.chatroom);
        }
        return "";
    }
    ...
    public String getGroupName(String identify) {
        for (String key : groups.keySet()) {
            for (GroupProfile item : groups.get(key)) {
                if (item.getIdentify().equals(identify)) {
                    return item.getName();
                }
            }
        }
    ...
    //获取群资料详情
    @Override
    public void onSuccess(List<TIMGroupDetailInfo> groupInfos) {
//        if (groupInfos != null) {
//            for (TIMGroupDetailInfo info:groupInfos) {
//                LogUtil.d("ailibin", "groupId: " + info.getGroupId()  //群组 ID
