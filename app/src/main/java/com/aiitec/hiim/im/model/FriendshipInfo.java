...
public class FriendshipInfo extends Observable implements Observer {
    ...
    private void refresh() {
        ...
        List<TIMFriendGroup> timFriendGroups = new ArrayList<>();
        try {
            timFriendGroups = TIMFriendshipProxy.getInstance().getFriendsByGroups(null);
        } catch (UnsatisfiedLinkError e) {
            e.printStackTrace();
        }
        if (timFriendGroups != null) {
    ...
    public boolean isFriend(String identify) {
        for (String key : friends.keySet()) {
            for (FriendProfile profile : friends.get(key)) {
                if (identify.equals(profile.getIdentify())) {
                    return true;
                }
            }
        }
