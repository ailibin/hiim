...
public abstract class CommonRecyclerViewAdapter<T> extends RecyclerView.Adapter<CommonRecyclerViewHolder> {
    ...
    public CommonRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = null;
        //说明是头部
        if (viewType == RecyclerView.INVALID_TYPE && mCurrentPosition != -1 && mCurrentPosition < headers.size()) {
...
//                ViewGroup.LayoutParams layoutParams = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
    ...
    /**
     * 通知某个item刷新(这里这个方法不起作用,还不知道什么原因)
     * @param position
     */
    public void notifyItemPosChanged(int position){
    ...
    public void onBindViewHolder(CommonRecyclerViewHolder h, int position) {
        ...
        if (position < size) {
            ...
            //如果需要设置监听
            if (isNeedSetClickListener(position)) {
    ...
    private class MyItemLongClickListenerAdapter implements View.OnLongClickListener {
        ...
        public boolean onLongClick(View v) {
            if (onRecyclerViewItemLongClickListener != null) {
                //回调方法
                onRecyclerViewItemLongClickListener.onItemLongClick(v, h.getAdapterPosition() - headers.size());
            }
            return true;
        }
    }
