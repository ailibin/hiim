...
import com.aiitec.imlibrary.presentation.viewfeatures.FriendInfoView;
...
//                Log.e("ailibin", "DecideUnReadCnt:" + arg0.getMeta().getDecideUnReadCnt());
//                Log.e("ailibin", "RecommendUnReadCnt:" + arg0.getMeta().getRecommendUnReadCnt());
     ...
     * @param callBack 回调
     */
    public static void setMyAvatarUrl(String imageUrl, TIMCallBack callBack) {
        TIMFriendshipManager.ModifyUserProfileParam param = new TIMFriendshipManager.ModifyUserProfileParam();
        param.setFaceUrl(imageUrl);
        TIMFriendshipManager.getInstance().modifyProfile(param, callBack);
    }
    ...
    public void changeFriendGroup(final String identify, final @Nullable String src, final @Nullable String dest) {
        if (friendshipManageView == null || src != null && dest != null && src.equals(dest)) {
            return;
        }
        if (src != null) {
